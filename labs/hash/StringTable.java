...
public class StringTable {
    ...
    {  
    	...
    	int recordHashCode= stringToHashCode(recordKey);
    	int recordIndex= toIndex(recordHashCode);
    	LinkedList<Record> currentBucketInsert=buckets[recordIndex]; 
    	for (int i=0; i<currentBucketInsert.size(); i++) {		
    		if (currentBucketInsert.get(i).key.equals(r.key)){
    			return false;
    		}
    	}
    	currentBucketInsert.add(r);
    	size ++;
    	return true;
    	}
    ...
    public Record find(String key) 
    {
    	...
    	int keyIndex= toIndex(keyHashCode);
    	LinkedList<Record> currentBucketFind=buckets[keyIndex];
    	for (int i=0; i<currentBucketFind.size(); i++) {		
    		if (currentBucketFind.get(i).key.equals(key)){
    			return currentBucketFind.get(i);
    		}
    	}
    	return null;
    ...
    public void remove(String key) 
    {
    	...
    	int keyHashCode= stringToHashCode(key);
    	int keyIndex= toIndex(keyHashCode);
    	LinkedList<Record> currentBucketRemove=buckets[keyIndex];
    	for (int i=0; i<currentBucketRemove.size(); i++) {		
    		if (currentBucketRemove.get(i).key.equals(key)){
    			currentBucketRemove.remove(currentBucketRemove.get(i));
    			size --;
    		}
    	}
    }
    ...
    private int toIndex(int hashcode)
    {
    	...
    	int tableIndex= Math.abs((int)(nBuckets*((.72423452351423*hashcode)%1)));
    	return tableIndex;
    }
